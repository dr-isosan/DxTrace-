...
1. **Skeleton Loaders:** Vaka değişimlerinde veya analiz API'si beklenirken, kullanıcıyı beyaz ekranla baş başa bırakmamak için orijinal düzeni (layout) taklit eden animasyonlu gri kutular gösterilir.
2. **Hover Geçişleri:** Menü ve liste öğelerinin üzerine gelindiğinde ince `translateX` kaymaları ve `box-shadow` aydınlanmaları ile dinamik bir hissiyat sağlanır.
...
Klinik sistemlerin zamanla "kendi hatasından öğrenmesi" kritik bir gereksinimdir. DxTrace bu döngüyü "Doktor Geri Bildirimi" paneliyle çözer:
